...
RESULTS_JSON="$SCRIPT_DIR/benchmark-results/results.json"
...
if command -v python3 &>/dev/null; then
    python3 "$SCRIPT_DIR/benchmark-results/parse_results.py" "$RESULTS_JSON"
else
    echo "Python 3 not found. Raw results saved to $RESULTS_JSON and $RESULTS_CSV"
fi

echo ""
echo "✅ Benchmarking complete! Results saved in backend/benchmark-results/"
