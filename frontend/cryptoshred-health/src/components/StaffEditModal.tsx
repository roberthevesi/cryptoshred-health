...
interface Props {
  ...
  onClose: () => void;
  onSuccess: () => void;
  staffUser: AdminUser | null;
}

export default function StaffEditModal({ isOpen, onClose, onSuccess, staffUser }: Props) {
  const [email, setEmail] = useState('');
  const [role, setRole] = useState<Role>('DOCTOR');
  const [passwordMode, setPasswordMode] = useState<'keep' | 'auto' | 'custom'>('keep');
  const [customPassword, setCustomPassword] = useState('');
  const [error, setError] = useState('');
  const [copied, setCopied] = useState(false);
  const [updatedResult, setUpdatedResult] = useState<AdminUser | null>(null);
  ...
  const mutation = useMutation({
    mutationFn: (data: AdminUserRequest) => {
      if (!staffUser) throw new Error('No staff user selected');
      return apiClient.put<AdminUser>(`/admin/users/${staffUser.id}`, data).then((r) => r.data);
    },
    onSuccess: (data) => {
      setError('');
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (passwordMode === 'auto') {
      passwordPayload = 'AUTO_GENERATE';
    } else if (passwordMode === 'custom' && customPassword.trim()) {
      passwordPayload = customPassword.trim();
    }
  ...
  const handleCopyCredentials = async () => {
    ...
    const textToCopy = updatedResult.temporaryPassword
      ? `CryptoShred Health Staff Credentials\nRole: ${updatedResult.role}\nEmail: ${updatedResult.email}\nNew Temporary Password: ${updatedResult.temporaryPassword}`
      : `CryptoShred Health Staff Credentials\nRole: ${updatedResult.role}\nEmail: ${updatedResult.email}`;
    try {
  ...
  // Handover card if password was reset
  if (updatedResult && updatedResult.temporaryPassword) {
    return createPortal(
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-xl max-w-lg w-full overflow-hidden animate-fade-in">
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 bg-emerald-50/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-slate-900">Staff Password Reset Successfully</h2>
                <p className="text-xs text-slate-500">New Credentials Issued</p>
              </div>
            </div>
            <button
              type="button"
          ...
          <div className="p-6 space-y-5">
            <div className="rounded-xl bg-emerald-50 border border-emerald-200/80 p-4 text-xs text-emerald-800 flex items-start gap-2.5">
              ...
              <span>
                The staff account password was regenerated. Provide these new credentials to the clinician.
              </span>
            </div>
            ...
            <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50/50 p-4">
              <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="mt-1">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                        updatedResult.role === 'DOCTOR'
                          ? 'bg-blue-100 text-blue-800 border border-blue-200'
                          : updatedResult.role === 'AUDITOR'
                ...
                <div>
                  ...
                  <p className="text-slate-900 font-medium text-xs mt-1">{updatedResult.email}</p>
                </div>
              </div>

              <div className="pt-2 border-t border-slate-200/60">
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-xs font-semibold text-slate-700">New Temporary Password</span>
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                    readOnly
                    value={updatedResult.temporaryPassword}
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xl max-w-lg w-full overflow-hidden animate-fade-in">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-slate-900">Edit Staff Account</h2>
              <p className="text-xs text-slate-500">Update clinician role or reset access credentials</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
            ...
            {/* Email */}
            <div>
              <label htmlFor="edit-staff-email" className="block text-xs font-semibold text-slate-700 mb-1.5">
            ...
            <div className="space-y-3 pt-2 border-t border-slate-200">
              <label className="block text-xs font-semibold text-slate-700">Password Management</label>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2.5 text-xs text-slate-700 cursor-pointer">
                  ...
                  />
                  <span>Keep existing password unchanged</span>
                </label>
                ...
                <label className="flex items-center gap-2.5 text-xs text-slate-700 cursor-pointer">
                  <input
                    ...
                    className="text-blue-600 focus:ring-blue-500"
                  />
                  <span>Set a custom new password</span>
                </label>
              </div>
