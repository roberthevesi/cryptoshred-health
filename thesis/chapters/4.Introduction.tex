...
Attempting physical deletion via relational SQL operations induces linear execution overhead $\mathcal{O}(N)$, scaling directly with the number of past visits $N$. Purging a long-term patient with $N = 10{,}000$ records forces PostgreSQL to lock dozens of related tables to resolve foreign-key cascades. This stalls active hospital writes while still leaving cold backup archives completely untouched.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Hypothesis $H_1$:} Deleting a 256-bit key inside a dedicated KMS executes with sub-$10\text{~ms}$ latency ($5.6\text{--}7.6\text{~ms}$ end-to-end) regardless of past visit volume $N$, achieving a $4\text{--}5\times$ latency speedup over relational foreign-key cascade deletions ($25\text{--}39\text{~ms}$) once $N \ge 10{,}000$ while maintaining strict $\mathcal{O}(1)$ time invariance.
    \end{itemize}

    \item \textbf{$RQ_2$ (Verifiability \& Post-Quantum Auditability):} 
    \textit{How can a distributed healthcare platform produce compact, tamper-proof deletion receipts that auditors can verify in logarithmic time ($\mathcal{O}(\log N)$) and that resist quantum attacks, without ever exposing plaintext clinical history?}
    \begin{itemize}
        \item \textbf{Hypothesis $H_2$:} Combining NIST FIPS 204 ML-DSA-65 post-quantum signatures with a binary Merkle tree allows independent auditors to verify deletion proofs in under $5\text{~ms}$ across registries with $100{,}000$ records, without needing direct database access.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Hypothesis $H_3$:} An automated Merkle tombstone reconciler auditing restored KMS keys against immutable ledgers achieves deterministic linear scaling ($\mathcal{O}(k)$) with sub-$5\text{~ms}$ per-key execution latency ($<50\text{~ms}$ for typical operational cohorts $k \le 30$), purging $100\%$ of resurrected keys before the backend begins accepting live hospital traffic.
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Hypothesis $H_4$:} Backing the architecture with a Redis L2 cache that automatically evicts shredded records allows the platform to sustain over $50{,}000\text{~req/s}$ on encrypted reads, while maintaining zero plaintext leaks across concurrent queries targeting shredded entities.
    \end{itemize}
\end{itemize}
