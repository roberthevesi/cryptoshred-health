...
As detailed in Listing~\ref{lst:patient-entity}, all identifiable demographic attributes carry Hibernate's \texttt{@Transient} annotation. This mapping completely omits fields such as \texttt{firstName}, \texttt{lastName}, \texttt{dateOfBirth}, and \texttt{nhsNumber} from the PostgreSQL DDL schema.
...
public class Patient {
    ...
    // Transient attributes: omitted from SQL schema, decrypted only in RAM
...
public String computeBlindIndex(String value, String salt) {
    ...
    try {
        ...
        // 2. Compute HMAC-SHA256 using KMS-derived salt
...
PostgreSQL builds standard $\text{B-Tree}$ indexes on \texttt{blind\_index\_nhs}, \texttt{blind\_index\_mrn}, and \texttt{blind\_index\_last\_name}. When a doctor searches for NHS Number \texttt{485-777-3456}, the backend computes the deterministic hash and dispatches the indexed SQL query shown in Equation~\ref{eq:sql-blind-search}, executing the match in $\mathcal{O}(1)$ time without reading unencrypted demographic data.
...
\begin{lstlisting}[language=Java, caption={Memory zeroization in \texttt{PatientService.java}.}, label={lst:memory-zeroization}]
byte[] dek = null;
...
try {
    ...
    // ... map remaining transient demographic fields ...
} catch (Exception e) {
    log.warn("Vault decryption failed for patient {}: key destroyed or invalid", patient.getPatientId());
    isShredded = true;
} finally {
    // 3. Explicit zeroization of ephemeral keys and decrypted plaintext
    if (dek != null) {
...
Executing \texttt{Arrays.fill(dek, (byte) 0)} and \texttt{Arrays.fill(decryptedBytes, (byte) 0)} immediately overwrites the raw 256-bit symmetric key and decrypted byte arrays with binary zeroes inside the \texttt{finally} block before the method frame unrolls. This explicitly minimizes the residency window of sensitive binary key material on the heap. However, as an engineering constraint of Java's managed runtime, converting decrypted bytes into domain strings (\texttt{new String(decryptedBytes, ...)}) and parsing them via Jackson's \texttt{ObjectMapper} allocates immutable \texttt{String} and DTO objects on the JVM heap. Because Java \texttt{String} internal character arrays cannot be safely mutated without unsafe reflection, these transient copies persist in memory until reclaimed and overwritten by HotSpot garbage collection cycles.
...
Linux kernel memory locking requires an architectural distinction between the KMS engine and Java runtimes. In the HashiCorp Vault container, the \texttt{IPC\_LOCK} capability (\texttt{cap\_add: [IPC\_LOCK]}) is used natively by Vault's Go runtime to execute the POSIX \texttt{mlock()} system call, preventing Vault's in-memory key ring from being paged to disk. In the Spring Boot container, however, the HotSpot JVM does not execute \texttt{mlockall()} on heap allocations; granting \texttt{IPC\_LOCK} merely provides the kernel capability without pinning Java heap pages. To eliminate swap leakage for transient JVM objects, the container runtime enforces anti-swap protection through Docker cgroup memory constraints that disable swap space entirely (\texttt{--memory-swap = --memory} with \texttt{mem\_swappiness = 0}). This ensures that physical host RAM never flushes idle JVM memory sectors to unencrypted swap partitions (requirement~SR3).
...
As illustrated in Listing~\ref{lst:attachment-service}, binary files are ingested via multipart HTTP requests (\texttt{POST /api/attachments/visit/\{visitId\}}). Uploads require \texttt{Role.DOCTOR} or explicit patient ownership authorization:

\begin{lstlisting}[language=Java, caption={Encrypted attachment ingestion and memory zeroization in \texttt{AttachmentService.java}.}, label={lst:attachment-service}]
@Transactional
public AttachmentResponse uploadAttachment(UUID visitId, MultipartFile file, String currentUserEmail) throws IOException {
    ...
    // 1. Retrieve visit DEK via Vault Transit KEK unwrap
    byte[] dek = null;
    ...
    try {
        ...
        // 2. Encrypt binary payload with AES-256-GCM
        encryptedPayload = envelopeEncryptionService.encrypt(file.getBytes(), dek);
    } finally {
        if (dek != null) {
            Arrays.fill(dek, (byte) 0); // Scoped zeroization
        }
    }
...
// 1. Query the latest activity strictly over active (non-shredded) visits
...
int retentionYears = retentionPolicyService.getRetentionPeriodYears();
LocalDateTime latestActivityDate = patient.getCreatedAt() != null ? patient.getCreatedAt() : LocalDateTime.now();
LocalDateTime maxActiveVisit = patientVisitRepository.findMaxActiveCreatedAtByPatient(patient.getId(), patient.getPatientId());

if (maxActiveVisit != null && maxActiveVisit.isAfter(latestActivityDate)) {
    latestActivityDate = maxActiveVisit;
}

LocalDateTime legalErasureEligibleDate = latestActivityDate.plusYears(retentionYears);
String retentionStatus = LocalDateTime.now().isAfter(legalErasureEligibleDate) ? "ELIGIBLE" : "PROTECTED";
\end{lstlisting}
...
\begin{enumerate}
    ...
    \item \textbf{Automated Follower Joining:} Nodes~2 and 3 poll Node~1 over the internal Docker network. Once Node~1 is initialized and unsealed, follower nodes execute:
...
Vault's \texttt{GET /v1/sys/health} endpoint returns HTTP status~\texttt{200} for the active leader, status~\texttt{429} for standby followers, and status~\texttt{503} for unsealed or offline nodes. HAProxy evaluates node health every $500\text{~ms}$. If the active leader fails, HAProxy detects the failure within two intervals ($1.0\text{~s}$) and redirects application traffic to the newly elected leader without dropping active clinical transactions.
...
\textit{CryptoShred Health} eliminates plaintext exposure by implementing an in-enclave zero-plaintext key rotation protocol orchestrated by \texttt{KeyManagementService} and exposed via \texttt{KeyManagementController} (\texttt{POST /api/keys/rotate}). The controller supports granular rotation across four distinct operational scopes:
\begin{enumerate}
...
for (EncryptionKey key : keysToProcess) {
    if (key.isInvalidated()) {
        skippedCount++; // Preserve shredded tombstones
        continue;
    }
    try {
        ...
        // 2. Re-wrap DEK under new KEK version inside Vault enclave
        ...
        int newVersion = key.getKeyVersion() + 1;
        ...
        // 4. Emit PATIENT_KEY_ROTATED event to Kafka audit log
        ...
        log.error("Failed to rotate key {}: {}", key.getKeyId(), e.getMessage());
        failedCount++;
    }
}
\end{lstlisting}
...
HashiCorp Vault Transit generates the classical signature directly within the isolated KMS using the standard \texttt{SHA256withRSA} algorithm. The private RSA key never leaves the KMS boundary. In parallel, the Spring Boot backend uses the BouncyCastle provider to generate a 3{,}309-byte post-quantum ML-DSA-65 lattice signature over a normalized JSON representation of the deletion certificate (with deterministically ordered fields to guarantee consistent verification). The prototype instantiates the algorithm identifier \texttt{"Dilithium3"} through the BouncyCastle Post-Quantum provider (\texttt{BCPQC}), which implements the round-3 CRYSTALS-Dilithium parameter set standardized in NIST FIPS~204 as ML-DSA-65.
...
This endpoint invokes \texttt{ProofSigningService.getPublicKeyPem()} to export the platform's 2048-bit RSA public key formatted as a standard PEM block. Vault Transit generates RSA signatures using the Probabilistic Signature Scheme (PSS). External Data Protection Officers (DPOs), patient representatives, and legal auditors can download this public key once, disconnect from the network, and verify the classical signature $\sigma_{\text{RSA}}$ on certificate $\mathcal{C}_{\text{erasure}}$ (Equation~\ref{eq:certificate-structure}) using standard OpenSSL tools with PSS padding:
\begin{lstlisting}[language=bash]
openssl dgst -sha256 -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:-1 -verify public-key.pem -signature cert.sig cert_payload.json
\end{lstlisting}
...
To prevent keys from being resurrected after restoring older backup snapshots (Section~\ref{section:kms-ha-tombstone}), \texttt{MerkleTombstoneReconciliationService} runs an automated audit whenever the application starts up. To guarantee resilience against pre-shred ($T_0 < T_1$) database restorations, the service harvests tombstones from two independent locations: relational entity records (\texttt{findByShreddedTrue()}) and external immutable WORM deletion receipts (\texttt{backups/deletion-receipt\_*.json}) read via non-blocking directory streams (\texttt{Files.newDirectoryStream}). The service gathers all destroyed key identifiers into an aggregated tombstone set $\mathcal{K}_{\text{tombstones}}$ and compares them against Vault's active keys.

Algorithm~\ref{alg:tombstone-reconciler} outlines this reconciliation procedure, which executes before the server accepts incoming network traffic:
...
By running this reconciliation audit before opening network ports, CryptoShred Health guarantees that restoring an older backup snapshot cannot resurrect previously destroyed encryption keys.
...
When exporting records for a crypto-shredded patient, the service strips all PII fields and injects a standard FHIR security extension using the canonical \texttt{REDACTED} code from the HL7 \texttt{v3-ObservationValue} code system:
\begin{lstlisting}[language=json]
"meta": {
  "tag": [
    {
...
\end{lstlisting}
Using the standard \texttt{REDACTED} vocabulary token guarantees strict schema validation across standard HL7 FHIR R4 validator suites, while the accompanying display descriptor preserves clear human-readable legal semantics for medical compliance auditors.
...
\begin{figure}[H]
    ...
    \caption{Primary Care Patient Census dashboard with 5-record pagination view, presenting blind-indexed NHS numbers, Medical Record Numbers, assigned General Practitioners, and dynamic statutory retention status badges.}
...
\begin{enumerate}
    \item \textbf{Core Persistence and KMS Tier (\texttt{default}):} Orchestrates the PostgreSQL~15 database, the 3-node HashiCorp Vault Raft cluster fronted by HAProxy, Apache Kafka~3.7 in KRaft consensus mode, and Redis~7. All datastores bind strictly to private container interfaces on the internal bridge network (\texttt{cryptoshred-net}) with zero public host port exposure.
