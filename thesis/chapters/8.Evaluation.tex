...
\begin{enumerate}
    ...
    \item \textbf{Formal Security Analysis and Functional Verification ($RQ_1, RQ_4$):} Cryptographic reduction proofs, JVM heap memory zeroization audits, automated test suite coverage (144/144 tests), and regulatory compliance under EDPB Guidelines 05/2014 and GDPR Article~17.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Crypto-Shredding (Vault Transit KMS):} Revoking the patient's 256-bit Key Encryption Key (KEK) via the HashiCorp Vault Transit operational API.
    \item \textbf{PostgreSQL Cascading SQL DELETE:} Executing parameterized cascading deletion queries across relational foreign-key hierarchies within PostgreSQL~15 over local loopback sockets.
    ...
    \item \textbf{H2 In-Memory JDBC DELETE:} Executing SQL table deletions against an embedded H2 database instance.
\end{enumerate}
...
The empirical measurements confirm Hypothesis~$H_1$. As historical visit volume expands from $N = 10$ to $N = 10,000$, PostgreSQL cascading deletion latency grows linearly from $839.58\,\mu\text{s}$ to $118,340.00\,\mu\text{s}$ ($118.34\text{~ms}$), driven by relational foreign-key integrity enforcement, row locking, and sequential write-ahead log (WAL) sync operations. In contrast, Vault Transit crypto-shredding exhibits asymptotic invariance, executing in $59.50\,\mu\text{s}$ at $N = 10,000$.

The initial latency increase from $N = 10$ ($31.49\,\mu\text{s}$) to $N = 100$ ($61.73\,\mu\text{s}$) is an artifact of the OpenJDK HotSpot tiered compilation model. During initial invocations, the C1 compiler executes interpreted and lightly instrumented code; once execution counts trigger the C2 tiered compiler, full JIT optimizations (loop unrolling, method inlining, and branch profiling) stabilize execution strictly around $59.50\text{--}65.17\,\mu\text{s}$ across all higher tiers.

Table~\ref{tab:deletion_complexity} also reflects embedded H2 JDBC execution ($13.42\text{--}36.56\,\mu\text{s}$), which outperforms crypto-shredding in microbenchmarking isolation. Because H2 runs entirely in-process on the local JVM heap, it circumvents inter-process context switches, socket allocations, and serialization boundaries. However, an in-process database cannot fulfill enterprise Key Management Service requirements: it lacks hardware security module (HSM) backing, independent cryptographic access boundaries, and distributed Raft consensus.

In isolated CPU computation, crypto-shredding achieves a $1{,}988.9\times$ speedup over PostgreSQL at $N = 10{,}000$. In production deployments, network transit and TLS handshakes dominate microsecond CPU savings. When evaluated end-to-end across HTTP REST interactions against a live HashiCorp Vault Raft cluster, crypto-shredding executes in $5.63\text{--}7.65\text{~ms}$, compared to $25.19\text{--}39.12\text{~ms}$ for PostgreSQL transactional cascades. This establishes an honest, real-world system speedup of $\approx 4\text{--}5\times$, entirely eliminating database table lock contention and disk I/O bottlenecks while guaranteeing mathematical irrecoverability.
...
To isolate the computational tax of zero-plaintext persistence, we benchmarked three distinct data ingestion pathways under identical testbed parameters:
\begin{enumerate}
    \item \textbf{Plaintext Baseline:} Standard in-memory object serialization without cryptographic transformations.
    \item \textbf{Local AES-256-GCM:} Local authenticated encryption with Authenticated Associated Data (AAD) binding using an unencrypted in-memory key buffer.
...
Does maintaining an authenticated binary Merkle DAG scale efficiently as cumulative erasure events grow over multi-year clinical operations, and can inclusion proofs be verified in sub-millisecond logarithmic time ($\mathcal{O}(\log N)$)? We hypothesize that leaf insertion operates in constant time ($\mathcal{O}(1)$) and directional inclusion proof verification executes in under $5\text{~ms}$ at $N = 100,000$ leaves.

\paragraph{2. Experimental Scaling Setup.}
We evaluated the persistent binary Merkle DAG implementation across four geometric orders of magnitude of deletion records: $N \in \{100, 1000, 10000, 100000\}$. For each tier, we measured: (1) leaf insertion latency, (2) directional proof path construction and verification time, and (3) full tree root reconstruction latency.
...
As documented in Table~\ref{tab:merkle-scaling} and Figure~\ref{fig:merkle-scaling}, individual leaf insertion operates in constant time ($0.133\text{--}0.179\,\mu\text{s}$) regardless of tree depth, requiring only the calculation of the initial leaf hash $H_{\text{leaf}}$ and append-only database persistence. Directional inclusion proof verification scales strictly logarithmically ($\mathcal{O}(\log N)$), requiring $2.48\text{~ms}$ to verify a historical deletion certificate against a tree containing $100{,}000$ leaves ($17$ tree levels).

This $2.48\text{~ms}$ verification latency reconciles directly with the microsecond CPU hashing bounds introduced in Section~\ref{section:theoretical-foundations}. The raw CPU computation for $17$ SHA-256 iterations requires merely $0.85\,\mu\text{s}$ in memory. However, full operational verification within the benchmark encompasses querying the database for sibling node digests, deserializing intermediate byte buffers into Java heap memory, traversing the 17-hop authentication path, and verifying both the classical RSA-2048 and post-quantum ML-DSA-65 signatures against the cached root.

The elevated sample standard deviations reported in Table~\ref{tab:merkle-scaling} (e.g., $2.478 \pm 3.085\text{~ms}$ for proof verification and $2{,}098.29 \pm 7{,}116.49\text{~ms}$ for root reconstruction at $100{,}000$ leaves) do not indicate negative latencies, but characterize a right-skewed, heavy-tailed distribution. Median latencies ($p_{50}$) remain tightly bounded ($0.82\text{~ms}$ for proof verification at $100\text{k}$ leaves), while occasional tail latency spikes occur due to secondary storage disk I/O when traversing unbuffered B-Tree leaf pages in PostgreSQL and transient stop-the-world JVM garbage collection (G1 young generation) pauses during large heap allocations. These empirical results validate Hypothesis~$H_2$, confirming that regulatory auditors can verify historical erasures in sub-$5\text{~ms}$ time without requiring access to clinical plaintext tables.
...
While microbenchmarks evaluate isolated algorithms, production EHR deployments must sustain concurrent traffic across multiple clinical wards. We evaluate the end-to-end distributed system using distributed k6 and asynchronous Node.js load engines, executing real HTTP REST interactions against the full containerized stack (Spring Boot~3.3.4, HashiCorp Vault Raft, PostgreSQL~15, Redis~7, Apache Kafka~3.7).
...
\begin{figure}[H]
    ...
    \caption{Macro system throughput (Requests per Second) across concurrency tiers ($1$ to $500\text{~VUs}$) across all four clinical testing scenarios.}
...
We launched an adversarial load test targeting exclusively crypto-shredded records across $2{,}377{,}883$ read requests (2.38~million), verifying that neither unkeyed JVM memory buffers, stale Redis cache keys, nor unsynchronized replicas disclose sensitive patient fields.
...
Can an automated tombstone reconciler audit restored KMS enclaves and eliminate $100\%$ of resurrected master keys resulting from cold backup restorations in linear time ($\mathcal{O}(k)$), with sub-$5\text{~ms}$ per-key execution latency, purging typical drift cohorts ($k \le 30$) in under $50\text{~ms}$ during disaster recovery boot sequences?
